...
use pandora_runtime::{
    AgentCheckpoint, AgentControlStop, AgentRunControl, ApprovalStore, ClaimedSubagent,
    SubagentPreparation, SubagentScope, SubagentStore,
};
use pandora_types::{
    EffectOutcome, EffectReceipt, ExecutionId, HarnessId, JobId, JobWorkerId, PermitId,
    PrincipalId, ReceiptId, RequestDigest, SessionId, SubagentBudgets, SubagentHarnessBinding,
    SubagentId, SubagentRequest, TenantId, Timestamp, WorkspaceId, hash_artifact,
};
...
impl CliSubagentFixture {
    ...
    fn new(changed_provider_binding: bool) -> Self {
        ...
        let persisted_provider_url = if changed_provider_binding {
            "http://127.0.0.1:9/v1"
        } else {
            &provider_url
        };
        ...
        let prepared = store
            .prepare(SubagentPreparation::new(
                ...
                Some(provider_binding_digest(persisted_provider_url)),
...
fn harness_binding_digest() -> String {
    let canonical = format!("harness-binding-v1\0{HARNESS_ID}\0{HARNESS_VERSION}\0");
