...
commands:
  ...
  package scaffold domain-harness --output <new-directory> [--id <id>] [--version <semver>] [--publisher <name>] [--gene <id>@<semver>] [--route-hint <hint>] | scaffold meta-harness --output <new-directory> [--id <id>] [--version <semver>] [--publisher <name>] [--domains <id>@<semver>,...] [--max-handoffs <1-64>] | admit --manifest <path> --artifact <path> | validate --manifest <path> --artifact <path> | sign --manifest <path> --artifact <path> --secret-name <vault-name> --output <path> [--yes] | keygen --publisher <name> --key-id <id> --secret-name <vault-name> | install <id> [version] [--registry <url>|--registry-profile <name>] [--token-env <name>] | install-github --repository <url> --commit <sha> --manifest <repo-path> --artifact <repo-path> [--token-env <name>] | list | inspect <id> <version> | enable|disable <id> <version> [--dry-run|--yes] | rollback <id> [--dry-run|--yes] | lock [--output <path>] | verify-lock [--lock <path>] | trust-root add|list|revoke | transparency list [--event-kind <kind>] [--outcome <allowed|denied>] [--limit <1-256>] | transparency inspect --sequence <id> | remove <id> <version> [--dry-run|--yes]
  ...
  evolution rollout configure --input <path> | score --input <path> | approve --input <path> | promote|pause|resume|reject|retry|rollback --id <proposal-id> --transition-id <sha256:digest> --actor <principal-id> [--reason <text>]
...
mod tests {
    ...
    fn help_lists_the_headless_job_surface() {
        ...
        assert!(
            usage.contains("evaluation suite register --id <id> --input <path> [--candidate <id>]")
        );
        ...
        assert!(usage.contains("evolution rollout configure --input <path>"));
        assert!(usage.contains("promote|pause|resume|reject|retry|rollback"));
