...
fn powershell() -> &'static str {
    ...
    $commands = if ($elements.Count -gt 1 -and $elements[1] -eq 'service') {
        ...
        'admit','validate','sign','keygen','install','install-github','list','inspect','enable','disable','rollback','lock','verify-lock','trust-root','remove'
...
fn bash() -> &'static str {
    ...
    if [[ "$previous" == "service" ]]; then
        ...
        COMPREPLY=( $(compgen -W 'admit validate sign keygen install install-github list inspect enable disable rollback lock verify-lock trust-root remove' -- "$current") )
...
complete -c pandora -f -n '__fish_seen_subcommand_from package' -a 'admit validate sign keygen install install-github list inspect enable disable rollback lock verify-lock trust-root remove'
