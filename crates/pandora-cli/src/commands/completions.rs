...
fn bash() -> &'static str {
    ...
    if [[ "$previous" == "service" ]]; then
        ...
        COMPREPLY=( $(compgen -W 'list start drain stop recover heartbeat reconcile' -- "$current") )
...
if [[ ${words[2]} == service ]]; then
    ...
    _arguments '3:supervisor command:(list start drain stop recover heartbeat reconcile)'
...
complete -c pandora -f -n '__fish_seen_subcommand_from fleet; and __fish_seen_subcommand_from supervisor' -a 'list start drain stop recover heartbeat reconcile'
