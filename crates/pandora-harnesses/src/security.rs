...
const SECURITY_GUIDE: &str = "The Security Domain is Pandora's Codex Security-style, evidence-first workflow surface. Security Assessment performs one bounded fixed-marker evidence pass without claiming complete scanner coverage. Security Scan inventories fixed high-signal security markers without claiming complete scanner coverage. Security Deep Scan searches a broader fixed marker set without claiming complete scanner coverage. Security Diff Scan searches changed-code and regression terminology without reviewing a specific revision. Security Audit searches boundary-sensitive source markers and returns evidence paths. Security Dependencies searches dependency declarations without claiming advisory or vulnerability coverage. Security Threat Model searches local trust-boundary and isolation evidence. Security Discovery records candidate source, control, sink, and reachability terminology without asserting a finding. Security Triage searches existing finding and proof terminology without assigning a verdict. Security Attack Path searches source, control, sink, impact, and privilege evidence without proving exploitability. Security Validation searches tests and validation evidence without running a scanner. Security Fix searches remediation planning terminology without changing code. Security Verify Fix searches regression and negative-control evidence without certifying a fix. Security Writeup searches disclosure fields without generating a vulnerability report. Security Track searches finding lifecycle fields without creating or mutating a finding record. Security Hardening searches local defensive-control evidence and does not change code. Security Policy searches local authorization terminology without certifying compliance. This is a native Pandora workflow, not a Codex Security workbench client or a complete vulnerability scanner. All filesystem effects require Pandora permits and receipts; process, network, package, and remediation actions require separately governed capabilities.";
