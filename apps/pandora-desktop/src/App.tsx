...
const navigation: Array<{ label: string; items: Array<{ id: ViewId; label: string; icon: IconName }> }> = [
  ...
  },
  {
    label: "Configure",
    items: [
      { id: "capabilities", label: "Harnesses & Genes", icon: "box" },
    ...
    <div className="app-shell">
      ...
      <main className="main-shell">
        ...
        {activeView === "command" ? (
          ...
          <CapabilitiesView harnesses={harnesses} runtimeStatus={runtimeStatus} />
        ) : activeView === "engines" ? (
...
function CommandView({ approvalPreview, selectedStep, onApprovalPreview, onApprovalClose, onSelectStep, runtimeStatus, selectedSession, lastRun, events, harnesses, runProfile, onRunProfileChange, onRun }: { approvalPreview: boolean; selectedStep: string; onApprovalPreview: () => void; onApprovalClose: () => void; onSelectStep: (id: string) => void; runtimeStatus: RuntimeStatus; selectedSession: RuntimeSessionDetail | null; lastRun: RuntimeRun | null; events: RuntimeEvent[]; harnesses: RuntimeHarness[]; runProfile: RunProfile; onRunProfileChange: (profile: RunProfile) => void; onRun: (task: string, profile: RunProfile) => Promise<void> }) {
  ...
  const evidenceValue = lastRun ? String(lastRun.receipt_count) : selectedSession ? String(selectedSession.event_count) : "—";
  ...
  return <div className="command-layout">
    <section className="core-column">
      ...
      <form className="composer-wrap" onSubmit={submit}><div className="composer"><button type="button" className="composer-add" aria-label="Context attachments unavailable" title="Context attachments are not available yet" disabled><Icon name="plus" size={17} /></button><textarea value={task} onChange={(event) => setTask(event.target.value)} onKeyDown={handleTaskKeyDown} placeholder="Ask Pandora to inspect, plan, or act…" aria-label="Pandora task" rows={1} /><div className="composer-actions"><label className="composer-profile"><span className="sr-only">Execution Harness</span><select value={runProfile} onChange={(event) => onRunProfileChange(event.target.value)} aria-label="Execution Harness">{profileOptions.map((profile) => <option value={profile.id} key={profile.id}>{profile.label}</option>)}</select></label><span className="composer-mode"><Icon name="spark" size={14} /> Governed run</span><button type="submit" className="send-button" aria-label="Send" disabled={!connected || !task.trim()}><Icon name="arrow" size={16} /></button></div></div><div className="composer-hint"><span>{runError || (connected ? "Ctrl/⌘ + Enter to send" : "Connect the local service in Connections")}</span><span>All effects require an exact permit</span></div></form>
    ...
    <Inspector steps={steps} approvalPreview={approvalPreview} hasLiveRun={Boolean(lastRun)} selectedStep={selectedStep} onApprovalPreview={onApprovalPreview} onApprovalClose={onApprovalClose} onSelectStep={onSelectStep} />
  </div>;
}
...
function Inspector({ steps, approvalPreview, hasLiveRun, selectedStep, onApprovalPreview, onApprovalClose, onSelectStep }: { steps: typeof authoritySteps; approvalPreview: boolean; hasLiveRun: boolean; selectedStep: string; onApprovalPreview: () => void; onApprovalClose: () => void; onSelectStep: (id: string) => void }) {
...
function CapabilitiesView({ harnesses, runtimeStatus }: { harnesses: RuntimeHarness[]; runtimeStatus: RuntimeStatus }) {
  ...
  return <div className="full-view"><PageHeader eyebrow="Runtime surface" title="Harnesses & Genes" description={connected ? "Installed Pandora Harnesses reported by the authenticated local runtime." : "Connect the local runtime to inspect installed Harnesses."} actions={<Chip tone={connected ? "green" : "neutral"} icon="box">{connected ? `${harnesses.length} Harnesses` : "Unavailable"}</Chip>} />{connected ? <div className="capability-toolbar"><span className="eyebrow">HARNESS TYPE</span><select value={kindFilter} onChange={(event) => setKindFilter(event.target.value)} aria-label="Filter Harnesses by type"><option value="all">All types</option>{kinds.map((kind) => <option value={kind} key={kind}>{kind.replaceAll("_", " ")}</option>)}</select><span className="capability-note">Gene identities are reported by the runtime.</span></div> : null}<div className="secondary-grid">{connected && visibleHarnesses.length ? visibleHarnesses.map((harness) => <Panel className="secondary-card" key={harness.id}><div className="secondary-icon secondary-icon-0"><Icon name="box" size={20} /></div><span className="eyebrow">{harness.runnable ? "RUNNABLE" : "BOUND"}</span><h3>{harness.name}</h3><p>{harness.kind} · {harness.gene_count} Genes · version {harness.version}</p>{harness.gene_ids?.length ? <div className="harness-gene-list">{harness.gene_ids.slice(0, 4).map((gene) => <span key={gene}>{gene}</span>)}{harness.gene_ids.length > 4 ? <span>+{harness.gene_ids.length - 4} more</span> : null}</div> : <span className="capability-note">Gene identities unavailable</span>}<span className="text-link">{harness.id}</span></Panel>) : <Panel className="secondary-card"><div className="secondary-icon secondary-icon-1"><Icon name="lock" size={20} /></div><span className="eyebrow">{connected ? "EMPTY" : "PREVIEW"}</span><h3>{connected ? "No Harnesses in this filter" : "Runtime connection required"}</h3><p>{connected ? "The authenticated service returned no matching Harnesses." : "This view does not fabricate Harness or Gene records."}</p></Panel>}</div></div>;
}

function ToolsView({ tools, runtimeStatus }: { tools: RuntimeTool[]; runtimeStatus: RuntimeStatus }) {
  const connected = runtimeStatus === "connected";
  return <div className="full-view"><PageHeader eyebrow="Runtime surface" title="Built-in Tools" description="Tool definitions exposed by Pandora’s ToolEngine. Tool metadata does not grant execution authority." actions={<Chip tone={connected ? "green" : "neutral"} icon="terminal">{connected ? `${tools.length} available` : "Unavailable"}</Chip>} /><div className="engine-notice"><Icon name="lock" size={16} /><span>{connected ? "Schemas and effect classifications are read-only here. Execution still requires the runtime’s policy and permit path." : "Connect the authenticated runtime to inspect built-in tools."}</span></div><div className="tool-grid">{connected && tools.length ? tools.map((tool) => <Panel className="tool-row" key={tool.id}><div><strong>{tool.name}</strong><small className="mono">{tool.id} · v{tool.version}</small></div><span className="tool-capability">{tool.capability}</span><span className="tool-operation">{tool.operation}</span></Panel>) : <Panel className="secondary-card"><div className="secondary-icon secondary-icon-1"><Icon name="lock" size={20} /></div><span className="eyebrow">{connected ? "EMPTY" : "UNAVAILABLE"}</span><h3>{connected ? "No tools reported" : "Runtime connection required"}</h3><p>{connected ? "The authenticated service returned an empty tool catalog." : "This surface does not fabricate tool definitions."}</p></Panel>}</div></div>;
}

function EnginesView({ engines, runtimeStatus }: { engines: RuntimeEngine[]; runtimeStatus: RuntimeStatus }) {
  const connected = runtimeStatus === "connected";
  return <div className="full-view"><PageHeader eyebrow="Architecture" title="Engines" description="Pandora’s engines are bounded modules around one governed execution path." actions={<Chip tone={connected ? "green" : "neutral"} icon="stack">{connected ? `${engines.length} reported` : "Unavailable"}</Chip>} /><div className="engine-notice"><Icon name="lock" size={16} /><span>{connected ? "Engine metadata is reported by the authenticated runtime. This describes ownership, not live health." : "Connect the authenticated runtime to inspect Pandora’s engine inventory."}</span></div><div className="secondary-grid">{connected && engines.length ? engines.map((engine, index) => <Panel className="secondary-card engine-card" key={engine.id}><div className={`secondary-icon secondary-icon-${index % 3}`}><Icon name={index < 2 ? "shield" : "stack"} size={20} /></div><span className="eyebrow">RUNTIME MODULE</span><h3>{engine.name}</h3><strong className="engine-role">{engine.role}</strong><p>{engine.authority}</p><span className="text-link">{engine.id}</span></Panel>) : <Panel className="secondary-card"><div className="secondary-icon secondary-icon-1"><Icon name="lock" size={20} /></div><span className="eyebrow">{connected ? "EMPTY" : "UNAVAILABLE"}</span><h3>{connected ? "No engines reported" : "Runtime connection required"}</h3><p>{connected ? "The authenticated service returned an empty engine inventory." : "This surface does not fabricate engine state."}</p></Panel>}</div></div>;
}
