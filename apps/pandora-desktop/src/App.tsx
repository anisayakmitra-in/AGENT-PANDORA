...
function Inspector({ steps, approvalPreview, lastRun, events, selectedSession, approval, approvalDetail, approvalInFlight, selectedStep, onApprovalPreview, onApprovalClose, onResolveApproval, onSelectStep }: { steps: typeof authoritySteps; approvalPreview: boolean; lastRun: RuntimeRun | null; events: RuntimeEvent[]; selectedSession: RuntimeSessionDetail | null; approval?: RuntimeApproval; approvalDetail?: string; approvalInFlight: boolean; selectedStep: string; onApprovalPreview: () => void; onApprovalClose: () => void; onResolveApproval: (allow: boolean) => Promise<void>; onSelectStep: (id: string) => void }) {
  ...
  return <aside className="inspector">
    ...
    {tab === "evidence" ? <div className="inspector-pane" role="tabpanel">
      ...
      <Panel className="context-panel"><div className="panel-heading"><div><span className="eyebrow">SCOPED CONTEXT</span><h3>{selectedSession?.session.workspace_id ?? "No workspace loaded"}</h3></div><Icon name="lock" size={17} /></div><div className="evidence-facts"><div><span>Session</span><strong className="mono">{selectedSession?.session.session_id ?? "—"}</strong></div><div><span>Tenant</span><strong>{selectedSession?.session.tenant_id ?? "—"}</strong></div><div><span>Principal</span><strong>{selectedSession?.session.principal_id ?? "—"}</strong></div><div><span>Disclosure</span><strong>Progressive · redacted</strong></div></div></Panel>
...
function CapabilitiesView({ harnesses, tools, runtimeStatus }: { harnesses: RuntimeHarness[]; tools: RuntimeTool[]; runtimeStatus: RuntimeStatus }) {
  ...
  return <div className="full-view harness-lab"><PageHeader eyebrow="Runtime surface" title="Harness Lab" description={connected ? "Inspect each Harness as a versioned capability boundary: Genes, extensions, authority, and execution evidence." : "Connect the local runtime to inspect installed Harnesses."} actions={<Chip tone={connected ? "green" : "neutral"} icon="box">{connected ? `${harnesses.length} Harnesses · ${tools.length} extensions` : "Unavailable"}</Chip>} />
    {connected ? <div className="capability-toolbar"><span className="eyebrow">HARNESS TYPE</span><select value={kindFilter} onChange={(event) => { setKindFilter(event.target.value); setSelectedHarnessId(""); }} aria-label="Filter Harnesses by type"><option value="all">All types</option>{kinds.map((kind) => <option value={kind} key={kind}>{kind.replaceAll("_", " ")}</option>)}</select><span className="capability-note">Read-only inventory from the authenticated runtime.</span></div> : null}
