...
function App() {
  ...
  const [memorySchedules, setMemorySchedules] = useState<RuntimeMemorySynthesisSchedule[]>([]);
  const [memoryScheduleRuns, setMemoryScheduleRuns] = useState<RuntimeMemorySynthesisScheduleRun[]>([]);
  const [memoryScheduleError, setMemoryScheduleError] = useState("");
  ...
  const [companion, setCompanion] = useState<CompanionSettings>(() => loadCompanionSettings(typeof window === "undefined" ? undefined : window.localStorage));
  ...
  const restoreDockToggleFocus = useRef(false);
  ...
  useEffect(() => {
    saveCompanionSettings(window.localStorage, companion);
  }, [companion]);
  ...
  useEffect(() => {
    if (!dockOpen && restoreDockToggleFocus.current) {
      restoreDockToggleFocus.current = false;
  ...
  useEffect(() => {
    if (!client) {
      ...
      setMemoryRecords([]);
      setMemorySchedules([]);
      setMemoryScheduleRuns([]);
      setMemoryScheduleError("");
  ...
  useEffect(() => {
    if (!client || runtimeStatus !== "connected" || activeView !== "memory") {
      return;
    }
    let cancelled = false;
    const refresh = async () => {
      try {
        const [schedules, runs] = await Promise.all([client.memorySchedules(), client.memoryScheduleRuns()]);
        if (!cancelled) {
          setMemorySchedules(schedules);
          setMemoryScheduleRuns(runs);
          setMemoryScheduleError("");
        }
      } catch (error: unknown) {
        if (!cancelled) setMemoryScheduleError(error instanceof Error ? error.message : "Could not load memory schedules");
      }
    };
    void refresh();
    const interval = window.setInterval(() => void refresh(), 3000);
  ...
  const createMemorySchedule = async (input: MemorySynthesisScheduleInput) => {
    if (!client) throw new Error("Connect to the local Pandora service first");
    const schedule = await client.createMemorySchedule(input);
    setMemorySchedules((current) => [...current.filter((item) => item.id !== schedule.id), schedule]);
    setMemoryScheduleError("");
  };

  const disableMemorySchedule = async (scheduleId: string) => {
    if (!client) throw new Error("Connect to the local Pandora service first");
    const schedule = await client.disableMemorySchedule(scheduleId);
    setMemorySchedules((current) => current.map((item) => item.id === schedule.id ? schedule : item));
  };
  ...
  const hideCommandDock = () => {
    restoreDockToggleFocus.current = true;
    setDockOpen(false);
  };

  const resetDockLayout = () => {
    setDockOpen(true);
    setDockPlacement("right");
    setDockSize("comfortable");
  };

  const companionState = deriveCompanionState({
    working: runInFlight || workspaceInspectionInFlight || browserInspectionInFlight,
    approvalRequired: lastRun?.status === "approval_required" || workspaceInspection?.status === "approval_required" || browserInspection?.status === "approval_required",
  ...
  return (
    <div className="app-shell">
      ...
      {companion.enabled ? <Companion settings={companion} state={companionState} onDisable={() => setCompanion((current) => ({ ...current, enabled: false }))} /> : null}
      ...
      <main ref={mainRef} id="pandora-main" className="main-shell" tabIndex={-1} aria-label={viewLabel(activeView) + " workspace"}>
        <TopBar activeView={activeView} runtimeStatus={runtimeStatus} dockOpen={dockOpen} dockPlacement={dockPlacement} dockToggleRef={dockToggleRef} onOpenPalette={() => setPaletteOpen(true)} onToggleDock={() => setDockOpen((current) => !current)} onMoveDock={() => { setDockOpen(true); setDockPlacement((current) => current === "right" ? "bottom" : "right"); }} />
        ...
        {activeView === "command" ? (
          <CommandView
            ...
            dockSize={dockSize}
            onDockOpenChange={(open) => open ? setDockOpen(true) : hideCommandDock()}
...
function TopBar({ activeView, runtimeStatus, dockOpen, dockPlacement, dockToggleRef, onOpenPalette, onToggleDock, onMoveDock }: { activeView: ViewId; runtimeStatus: RuntimeStatus; dockOpen: boolean; dockPlacement: WorkspaceDockPlacement; dockToggleRef: RefObject<HTMLButtonElement | null>; onOpenPalette: () => void; onToggleDock: () => void; onMoveDock: () => void }) {
...
function CommandView({ selectedStep, onSelectStep, runtimeStatus, selectedSession, lastRun, lastRunRequest, events, harnesses, providers, runInFlight, workspaceInspection, workspaceInspectionInFlight, browserInspection, browserInspectionInFlight, runProfile, onRunProfileChange, runProvider, runModel, dockOpen, dockPlacement, dockSize, onDockOpenChange, onDockPlacementChange, onDockSizeChange, onRunProviderChange, onRunModelChange, onRun, onRetryRun, onResolveApproval, onInspectWorkspace, onResolveWorkspaceInspection, onInspectBrowser, onResolveBrowserInspection }: CommandViewProps) {
  ...
  return <div className={`command-layout dock-${dockOpen ? dockPlacement : "closed"} dock-size-${dockSize}`} data-dock-placement={dockOpen ? dockPlacement : "closed"} aria-busy={runInFlight}>
    <section className="core-column">
      ...
      {!dockOpen && (lastRun?.status === "approval_required" || workspaceInspection?.status === "approval_required" || browserInspection?.status === "approval_required") ? <section className="hidden-dock-approval" aria-label="Pending approval"><div className="hidden-dock-approval-heading"><span className="eyebrow">INSPECTOR HIDDEN</span><strong>Approvals remain visible and unresolved</strong><button className="text-link" type="button" onClick={() => onDockOpenChange(true)}>Show full inspector <Icon name="arrow" size={13} /></button></div>{lastRun?.status === "approval_required" ? <div className="hidden-dock-approval-item"><span className="eyebrow">RUN APPROVAL</span><ApprovalPanel approval={lastRun.approval} approvalDetail={lastRun.status_detail} approvalInFlight={runInFlight} onResolveApproval={onResolveApproval} /></div> : null}{workspaceInspection?.status === "approval_required" ? <div className="hidden-dock-approval-item"><span className="eyebrow">WORKSPACE APPROVAL</span><ApprovalPanel approval={workspaceInspection.approval} approvalDetail={workspaceInspection.status_detail} approvalInFlight={workspaceInspectionInFlight} onResolveApproval={onResolveWorkspaceInspection} /></div> : null}{browserInspection?.status === "approval_required" ? <div className="hidden-dock-approval-item"><span className="eyebrow">NETWORK APPROVAL</span><ApprovalPanel approval={browserInspection.approval} approvalDetail={browserInspection.status_detail} approvalInFlight={browserInspectionInFlight} onResolveApproval={onResolveBrowserInspection} /></div> : null}</section> : null}
    </section>
...
function ApprovalPanel({ approval, approvalDetail, approvalInFlight, onResolveApproval }: { approval?: RuntimeApproval; approvalDetail?: string; approvalInFlight: boolean; onResolveApproval: (allow: boolean) => Promise<void> }) {
...
function Inspector({ steps, lastRun, events, selectedSession, runtimeStatus, approval, approvalDetail, approvalInFlight, workspaceInspection, workspaceInspectionInFlight, browserInspection, browserInspectionInFlight, selectedStep, dockPlacement, dockSize, onClose, onPlacementChange, onSizeChange, onResolveApproval, onInspectWorkspace, onResolveWorkspaceInspection, onInspectBrowser, onResolveBrowserInspection, onSelectStep }: { steps: typeof authoritySteps; lastRun: RuntimeRun | null; events: RuntimeEvent[]; selectedSession: RuntimeSessionDetail | null; runtimeStatus: RuntimeStatus; approval?: RuntimeApproval; approvalDetail?: string; approvalInFlight: boolean; workspaceInspection: RuntimeRun | null; workspaceInspectionInFlight: boolean; browserInspection: RuntimeRun | null; browserInspectionInFlight: boolean; selectedStep: string; dockPlacement: WorkspaceDockPlacement; dockSize: WorkspaceDockSize; onClose: () => void; onPlacementChange: (placement: WorkspaceDockPlacement) => void; onSizeChange: (size: WorkspaceDockSize) => void; onResolveApproval: (allow: boolean) => Promise<void>; onInspectWorkspace: (task: string) => Promise<void>; onResolveWorkspaceInspection: (allow: boolean) => Promise<void>; onInspectBrowser: (url: string) => Promise<void>; onResolveBrowserInspection: (allow: boolean) => Promise<void>; onSelectStep: (id: string) => void }) {
  const restoredWorkspaceTask = workspaceInspection?.selected_gene?.startsWith("workspace.")
    ? workspaceInspection.selected_gene.slice("workspace.".length)
    : "";
  const [tab, setTab] = useState<InspectorTab>(browserInspection ? "browser" : workspaceInspection ? "work" : "flow");
  const [optionsOpen, setOptionsOpen] = useState(false);
  const [workSurface, setWorkSurface] = useState<WorkSurface>(restoredWorkspaceTask ? workSurfaceForTask(restoredWorkspaceTask) : "files");
  const [workspaceTask, setWorkspaceTask] = useState(restoredWorkspaceTask);
  ...
  return <aside className="inspector" aria-label="Workspace inspector">
    ...
    {tab === "evidence" ? <div id="run-inspector-panel" className="inspector-pane" role="tabpanel" aria-labelledby={"run-inspector-tab-" + tab}>
      ...
      <ApprovalPanel approval={approval} approvalDetail={approvalDetail} approvalInFlight={approvalInFlight} onResolveApproval={onResolveApproval} />
    ...
    <Panel className="operations-overview">
      ...
      {operations ? <>
        ...
        <div className="operations-evidence-grid">
          ...
          <section><div className="operations-section-heading"><span className="eyebrow">AGGREGATE BUDGET</span><strong>Measured use and reserved capacity</strong></div>{operations.aggregate_budgets.run_count ? <dl className="operations-budget"><div><dt>Tokens</dt><dd>{operations.aggregate_budgets.consumed.tokens.toLocaleString()} used · {operations.aggregate_budgets.reserved.tokens.toLocaleString()} reserved · {operations.aggregate_budgets.remaining.tokens.toLocaleString()} available / {operations.aggregate_budgets.ceiling.tokens.toLocaleString()} ceiling</dd></div><div><dt>Tools</dt><dd>{operations.aggregate_budgets.consumed.tools.toLocaleString()} used · {operations.aggregate_budgets.reserved.tools.toLocaleString()} reserved · {operations.aggregate_budgets.remaining.tools.toLocaleString()} available / {operations.aggregate_budgets.ceiling.tools.toLocaleString()} ceiling</dd></div><div><dt>Elapsed</dt><dd>{operations.aggregate_budgets.consumed.elapsed_ms.toLocaleString()} ms used · {operations.aggregate_budgets.reserved.elapsed_ms.toLocaleString()} ms reserved · {operations.aggregate_budgets.remaining.elapsed_ms.toLocaleString()} ms available</dd></div><div><dt>Actual cost</dt><dd>{operations.aggregate_budgets.consumed.cost_micros === null ? "Unknown" : `${operations.aggregate_budgets.consumed.cost_micros.toLocaleString()} µ`} · {operations.aggregate_budgets.consumed.unknown_cost_receipts} unknown receipt{operations.aggregate_budgets.consumed.unknown_cost_receipts === 1 ? "" : "s"} · {operations.aggregate_budgets.consumed.enforced_cost_micros.toLocaleString()} µ enforced</dd></div></dl> : <p className="operations-empty">No governed run budget has been recorded in this scope.</p>}</section>
          <section><div className="operations-section-heading"><span className="eyebrow">LEASE CEILINGS</span><strong>Fleet capacity envelopes</strong></div><dl className="operations-budget"><div><dt>Tokens</dt><dd>{operations.budget_ceilings.max_tokens.toLocaleString()}</dd></div><div><dt>Tools</dt><dd>{operations.budget_ceilings.max_tools.toLocaleString()}</dd></div><div><dt>Duration</dt><dd>{operations.budget_ceilings.max_duration_seconds.toLocaleString()}s</dd></div><div><dt>Cost</dt><dd>{operations.budget_ceilings.max_cost_micros.toLocaleString()} µ</dd></div></dl></section>
...
function MemoryWorkspace({
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!selectedSession) {
      setFormError("Select a session before creating a schedule.");
      return;
    }
    const intervalSeconds = Number.parseInt(interval, 10);
    if (!name.trim() || !memoryId.trim() || !summary.trim() || !Number.isInteger(intervalSeconds) || intervalSeconds < 60) {
      setFormError("Name, memory ID, summary, and an interval of at least 60 seconds are required.");
      return;
    }
    setInFlight(true);
    setFormError("");
    try {
      ...
      setFormError(submitError instanceof Error ? submitError.message : "Could not create memory schedule");
    } finally {
      setInFlight(false);
    }
  };
  const disable = async (scheduleId: string) => {
    setInFlight(true);
    setFormError("");
    try {
      await onDisable(scheduleId);
    } catch (disableError: unknown) {
      setFormError(disableError instanceof Error ? disableError.message : "Could not disable memory schedule");
    } finally {
      setInFlight(false);
    }
  };
...
function PackageAuthoring() {
  ...
  const preview = useMemo(() => {
    ...
    if (kind === "meta_harness" && (!Number.isInteger(handoffLimit) || handoffLimit < 1 || handoffLimit > 64)) errors.push("Meta handoff limit must be an integer from 1 to 64.");
  ...
  return <div className="package-authoring">
    ...
    <div className="package-boundary" aria-label="Meta Harness starter kit"><Icon name="stack" size={14} /><span>For composition-only coordination, run <code>pandora package scaffold meta-harness --output &lt;new-directory&gt;</code>. The TUI exposes the same local-only guidance with <code>/meta-starter</code>.</span></div>
    <div className="package-boundary" aria-label="Gene pack examples"><Icon name="code" size={14} /><span>Evaluate static, bounded-read, and effect-request examples under <code>sdk/gene-pack</code>. Use <code>/gene-pack</code> in the TUI for the lifecycle and authority reminder.</span></div>
    ...
    <div className="package-manager-grid">
      ...
      <div className="package-console">
        {selectedPackage ? <div className="package-inspection">
          ...
          {selectedPackage.kind === "meta_harness" && selectedPackage.meta_composition ? <div className="package-route-preview" aria-label="Meta composition inspection"><div className="package-route-preview-heading"><div><span className="eyebrow">COMPOSITION CONTRACT</span><strong>Declared Domain handoff boundary</strong></div><Chip tone="blue">{selectedPackage.meta_composition.max_handoffs} handoffs max</Chip></div><div className="package-route-claims">{selectedPackage.meta_composition.allowed_domains.map((domain) => { const dependency = selectedPackage.dependencies.find((candidate) => candidate.id === domain); return <div key={domain}><span className="mono">{domain}</span><Chip tone={dependency && !dependency.optional ? "green" : "gold"}>{dependency ? `${dependency.version}${dependency.optional ? " optional" : " exact"}` : "version unresolved"}</Chip><small>Composition member only; no effect authority.</small></div>; })}</div><p className="package-route-note">Plans naming another Domain or exceeding the handoff ceiling fail before effect execution. This profile owns no Genes, approves nothing, and cannot issue a permit.</p></div> : null}
          {selectedPackage.kind === "gene" && selectedPackage.gene_contract ? <div className="package-route-preview" aria-label="Gene contract inspection"><div className="package-route-preview-heading"><div><span className="eyebrow">GENE CONTRACT</span><strong>{selectedPackage.gene_contract.execution.replaceAll("_", " ")}</strong></div><Chip tone={selectedPackage.gene_contract.capabilities.length ? "gold" : "green"}>{selectedPackage.gene_contract.capabilities.length ? `${selectedPackage.gene_contract.capabilities.length} declared` : "no downstream effect"}</Chip></div><div className="package-route-claims"><div><span>Capabilities</span><strong className="mono">{selectedPackage.gene_contract.capabilities.join(", ") || "none"}</strong><small>{selectedPackage.gene_contract.approval_required ? "Explicit approval required by this contract." : "Policy still evaluates every typed request."}</small></div>{(selectedPackage.owning_domains ?? []).map((domain) => <div key={`${domain.id}@${domain.version}`}><span className="mono">{domain.id}@{domain.version}</span><Chip tone="blue">owning Domain</Chip><small>{domain.state}; ownership grants no effect authority.</small></div>)}</div><p className="package-route-note">Publisher {selectedPackage.provenance?.publisher ?? selectedPackage.publisher} · artifact {selectedPackage.provenance?.artifact_verified ? "SHA-256 verified" : "verification unavailable"}. The Gene cannot call an executor, approve itself, or issue a permit.</p></div> : null}
...
function SettingsView({ appearance, onAppearanceChange, companion, onCompanionChange, runtimeStatus, health, native, endpoint, dockOpen, dockPlacement, dockSize, onDockOpenChange, onDockPlacementChange, onDockSizeChange, onResetDockLayout, onOpenView }: { appearance: AppearanceSettings; onAppearanceChange: (nextAppearance: AppearanceSettings) => void; companion: CompanionSettings; onCompanionChange: (nextCompanion: CompanionSettings) => void; runtimeStatus: RuntimeStatus; health: RuntimeHealth | null; native: boolean; endpoint: string; dockOpen: boolean; dockPlacement: WorkspaceDockPlacement; dockSize: WorkspaceDockSize; onDockOpenChange: (open: boolean) => void; onDockPlacementChange: (placement: WorkspaceDockPlacement) => void; onDockSizeChange: (size: WorkspaceDockSize) => void; onResetDockLayout: () => void; onOpenView: (view: ViewId) => void }) {
  ...
  const updateCompanion = <Key extends keyof CompanionSettings>(key: Key, value: CompanionSettings[Key]) => {
    onCompanionChange({ ...companion, [key]: value });
  };
  const detail = section === "appearance"
    ? <AppearanceSettings appearance={appearance} onChange={updateAppearance} companion={companion} onCompanionChange={updateCompanion} />
    : section === "workspace"
...
function AppearanceSettings({ appearance, onChange, companion, onCompanionChange }: { appearance: AppearanceSettings; onChange: <Key extends keyof AppearanceSettings>(key: Key, value: AppearanceSettings[Key]) => void; companion: CompanionSettings; onCompanionChange: <Key extends keyof CompanionSettings>(key: Key, value: CompanionSettings[Key]) => void }) {
...
const companionStateLabels: Record<CompanionState, string> = {
  idle: "Idle and ready",
  working: "Governed work in progress",
  waiting: "Waiting for an exact approval",
  success: "Latest run completed",
  failure: "Latest run did not complete",
};

function Companion({ settings, state, onDisable }: { settings: CompanionSettings; state: CompanionState; onDisable: () => void }) {
  return <aside className={`pandora-companion position-${settings.position} scale-${settings.scale} motion-${settings.motion}`} data-state={state} aria-label="Pandora companion"><div className="companion-status" role="status" aria-live="polite"><img src={`/companions/${builtInCompanionManifest.assets[state]}`} alt="" /><span><strong>Pandora Orbit</strong><small>{companionStateLabels[state]}</small></span></div><button type="button" aria-label="Disable Pandora companion" onClick={onDisable}>×</button></aside>;
}

function CompanionSettingsPanel({ settings, onChange }: { settings: CompanionSettings; onChange: <Key extends keyof CompanionSettings>(key: Key, value: CompanionSettings[Key]) => void }) {
  const [previewState, setPreviewState] = useState<CompanionState>("idle");
  const positions: readonly CompanionPosition[] = ["bottom-left", "bottom-right"];
  const scales: readonly CompanionScale[] = ["small", "medium", "large"];
  const motions: readonly CompanionMotion[] = ["system", "static"];
  return <Panel className="settings-panel companion-settings"><div className="panel-heading"><div><span className="eyebrow">OPTIONAL LOCAL COMPANION</span><h3>Pandora Orbit</h3></div><Chip tone={settings.enabled ? "green" : "neutral"}>{settings.enabled ? "On" : "Off by default"}</Chip></div><p className="settings-copy">Orbit reads only the typed public state shown below. It cannot read prompts, files, memories, tool output, or hidden reasoning.</p><div className="settings-control-group" role="group" aria-label="Companion visibility"><button type="button" aria-pressed={settings.enabled} onClick={() => onChange("enabled", true)}>On</button><button type="button" aria-pressed={!settings.enabled} onClick={() => onChange("enabled", false)}>Off</button></div><div className="companion-preview" data-state={previewState} data-motion={settings.motion}><img src={`/companions/${builtInCompanionManifest.assets[previewState]}`} alt="" /><div><span className="eyebrow">FIXTURE PREVIEW</span><strong>{companionStateLabels[previewState]}</strong><small>No runtime or prompt data is used.</small></div></div><div className="companion-state-options" role="group" aria-label="Companion preview state">{companionStates.map((state) => <button type="button" aria-pressed={previewState === state} onClick={() => setPreviewState(state)} key={state}>{state}</button>)}</div><div className="settings-control-label"><span>Position</span><small>Local display preference</small></div><div className="settings-control-group" role="group" aria-label="Companion position">{positions.map((position) => <button type="button" aria-pressed={settings.position === position} onClick={() => onChange("position", position)} key={position}>{position.replace("-", " ")}</button>)}</div><div className="settings-control-label"><span>Scale</span><small>Local display preference</small></div><div className="settings-control-group" role="group" aria-label="Companion scale">{scales.map((scale) => <button type="button" aria-pressed={settings.scale === scale} onClick={() => onChange("scale", scale)} key={scale}>{scale}</button>)}</div><div className="settings-control-label"><span>Motion</span><small>System reduction always wins</small></div><div className="settings-control-group" role="group" aria-label="Companion motion">{motions.map((motion) => <button type="button" aria-pressed={settings.motion === motion} onClick={() => onChange("motion", motion)} key={motion}>{motion}</button>)}</div><div className="settings-proof-row"><Icon name="lock" size={13} /><span>Companion packs are declarative local image manifests. Remote, executable, symlinked, traversal, and oversized assets fail closed.</span></div></Panel>;
}
