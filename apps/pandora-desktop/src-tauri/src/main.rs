...
fn pandora_rpc(
    ...
    if !matches!(
        method.as_str(),
        "runtime.health" | "runtime.capabilities" | "runtime.providers" | "runtime.engines" | "runtime.tools" | "session.list" | "session.inspect" | "session.events" | "session.memory" | "run.execute"
    ) {
