...
The built-in `coding-domain` Harness owns sixteen Genes. Eleven are narrow execution
primitives:
...
aliases are `/coding`, `/read`, `/search`, `/patch`, `/verify`, `/test`, `/format`, `/lint`, `/build`, `/status`, `/diff`, `/review`,
