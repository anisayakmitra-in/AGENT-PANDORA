...
| 11. Adversarial resilience | 55-65% | path confinement, symlink checks, secret redaction, replay protection, exact signatures and hashes, fail-closed package and permit checks, adversarial evaluation primitives, source-labelled deterministic quarantine for high-confidence instruction-shaped tool and adapter output, replayable hostile-output corpus coverage | prompt-injection corpus across every input source, publisher trust roots and revocation, fuzzing and hostile multi-agent handoff suites |
...
- connect task-backed prompt, Skill, workflow, and WebAssembly Gene evaluations to the registry; the current runner executes validated suite-bound golden inputs;
