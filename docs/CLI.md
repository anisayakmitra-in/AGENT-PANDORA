...
Tool output is framed as source-labelled untrusted data before it reaches a provider.
The same quarantine envelope is used for adapter results such as MCP, package,
repository, document, and issue content; it cannot serve as policy, authorization,
or approval. A restored tool message without a bound tool-call ID is rejected.
