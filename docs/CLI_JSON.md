...
| `pandora evaluation suite ... --json` | `evaluation suite` | register/inspect: `id`, `digest`, `definition_bytes`, `created_at`, `durability`; register with `--candidate` also returns `candidate_id` and `review_gate`; run: `total`, `passed`, `failed`, `digest`, `cases` with typed `target` and `task`; list: `suites`, `count`, `durability`
